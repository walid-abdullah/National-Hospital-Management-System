...
try {
    $query = "SELECT m.id as record_id, p.name AS patient_name, d.name as doctor_name, m.diagnosis, m.treatment, m.visit_date 
...
<head>
    ...
    <title>Medical Records - NHIMS</title>
...
<body class="bg-slate-50 text-slate-800 antialiased selection:bg-blue-200 selection:text-blue-900 flex flex-col min-h-screen transition-colors duration-300 dark:bg-gray-900 dark:text-gray-100">
    ...
    <div class="max-w-6xl mx-auto animate-fade-in-up p-6 mt-6">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Medical Records</h2>
            <a href="add_medical_record.php" class="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white shadow-md hover:shadow-lg transform hover:-translate-y-0.5 transition-all duration-200 px-5 py-2 rounded-lg font-medium shadow transition">
                + Add Record
            </a>
        </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    <?php if(!empty($records)): ?>
                        ...
                        <tr class="hover:bg-blue-50 dark:hover:bg-slate-700 transition-colors duration-200">
                            ...
                            <td class="p-4 text-sm font-medium text-blue-600"><?php echo htmlspecialchars($rec['doctor_name']); ?></td>
                            <td class="p-4 text-sm text-gray-600 dark:text-gray-300"><?php echo htmlspecialchars($rec['diagnosis']); ?></td>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="6" class="p-6 text-center text-gray-500 dark:text-gray-400">No medical records found.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
